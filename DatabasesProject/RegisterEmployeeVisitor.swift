...
class RegisterEmployeeVisitor: UIViewController, UITableViewDelegate, UITableViewDataSource, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func register(_ sender: Any) {
        if let first = firstName.text, let last = lastName.text, let uname = username.text, let phoneNum = phone.text, let addr = address.text, let cit = city.text, let zp = zip.text {
            ...
            if st != -1 && utype != -1 {
                let user: EmployeeVisitor = EmployeeVisitor(first: first, last: last, uname: uname, emailAddr: emailList, phoneNum: phoneNum, addr: addr, cit: cit, st: states[st], zp: zp, utype: utype)
                if let pass = password.text, let conf = confirmPassword.text {
                    if pass.elementsEqual(conf) {
                        if (Model.getInstance().register(user: user, password: pass)) {
                            performSegue(withIdentifier: "register_employee_visitor_to_nav", sender: self)
                        }
                    } else {
