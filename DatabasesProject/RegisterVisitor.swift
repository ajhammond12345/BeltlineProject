...
class RegisterVisitor: UIViewController , UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func register(_ sender: Any) {
        if let first = firstName.text, let last = lastName.text, let uname = username.text {
            let user: Visitor = Visitor(first: first, last: last, uname: uname, emailAddresses: emailList)
            if let pass = password.text, let conf = confirmPassword.text {
                if pass.elementsEqual(conf) {
                    if (Model.getInstance().register(user: user, password: pass)) {
                        performSegue(withIdentifier: "register_visitor_to_nav", sender: self)
                    }
                }
            } else {
